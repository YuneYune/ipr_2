...
Функционал: REST заглушки
  ...
  @post
  Сценарий: Делая POST запрос с данными пропуска, в ответе получаем id пропуска
  ...
  @get
  Сценарий: [01] Делая GET запрос с существующим id действующего пропуска, получаем всю инф. о пропуске
    * Послали POST на URL "http://localhost:1488/pass/" с параметрами:
      | key             | value      |
      | first_name      | Gussi      |
      | last_name       | Gang       |
    ...
    * Убедились, что в теле ответа есть вся инфа о пропуске

  @200
  Сценарий: [02] Делая GET запрос с существующим id действующего пропуска, получаем 200 статус код
    * Послали POST на URL "http://localhost:1488/pass/" с параметрами:
      | key             | value      |
      | first_name      | Gussi      |
      | last_name       | Gang       |
  ...
  @410
  Сценарий: [04] Делая GET запрос с существующим id, но с просроченной датой пропуска, получаем 410 статус код
    * Послали GET "http://localhost:1488/pass/FB565247-8FCC-E248-B5F6-01F5CE351488" запрос
    * Убедились, что http status code == 410


  @delete
  Сценарий: Делая DELETE запрос с существующим id пропуска, пропуск удаляется из БД
    * Послали POST на URL "http://localhost:1488/pass/" с параметрами:
      | key             | value      |
      | first_name      | Gussi      |
      | last_name       | Gang       |
  ...
  @put
  Сценарий: Делая PUT запрос с существующим id пропуска, данные пропуска меняются в БД
    * Послали POST на URL "http://localhost:1488/pass/" с параметрами:
      | key             | value      |
      | first_name      | Gussi      |
      | last_name       | Gang       |
